...
MPU6500::MPU6500(SPI_HandleTypeDef *hspi, const GPIO &chip_select,
  ...
  for (size_t i = 0; i < init_len; ++i) {
    ...
    if (tmp != init_data[i][1])
      bsp_error_handler(__FUNCTION__, __LINE__,
                        "imu register incorrect initialization");
  }
  // setup interrupt callback
  RM_ASSERT_FALSE(mpu6500, "Repteated intialization of MPU6500");
  mpu6500 = this;
  HAL_SPI_RegisterCallback(hspi, HAL_SPI_TX_RX_COMPLETE_CB_ID,
...
void MPU6500::UpdateData() {
  ...
  HAL_SPI_TransmitReceive_DMA(hspi_, io_buff_, io_buff_,
                              MPU6500_SIZEOF_DATA + 1);
}
...
IST8310::IST8310(I2C_HandleTypeDef *hi2c, uint16_t int_pin,
                 GPIO_TypeDef *rst_group, uint16_t rst_pin, IMU_typeC *imu)
    : GPIT(int_pin) {
  hi2c_ = hi2c;
...
bool IST8310::IsReady() {
  return HAL_I2C_IsDeviceReady(hi2c_, IST8310_IIC_ADDRESS << 1, 1, 100) ==
         HAL_OK;
}
...
void IST8310::ist8310_read_over(uint8_t *status_buf,
...
uint8_t IST8310::ist8310_IIC_read_single_reg(uint8_t reg) {
  uint8_t res = 0;
  HAL_I2C_Mem_Read(hi2c_, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                   &res, 1, 10);
  return res;
}

void IST8310::ist8310_IIC_write_single_reg(uint8_t reg, uint8_t data) {
  HAL_I2C_Mem_Write(hi2c_, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                    &data, 1, 10);
}

void IST8310::ist8310_IIC_read_muli_reg(uint8_t reg, uint8_t *buf,
                                        uint8_t len) {
  HAL_I2C_Mem_Read(hi2c_, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                   buf, len, 10);
}

void IST8310::ist8310_IIC_write_muli_reg(uint8_t reg, uint8_t *data,
                                         uint8_t len) {
  HAL_I2C_Mem_Write(hi2c_, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                    data, len, 10);
}
...
BMI088::BMI088(SPI_HandleTypeDef *hspi, GPIO_TypeDef *CS_ACCEL_Port,
...
void BMI088::BMI088_accel_read_muli_reg(uint8_t reg, uint8_t *buf,
                                        uint8_t len) {
  BMI088_ACCEL_NS_L();
  BMI088_read_write_byte(reg | 0x80);
...
Accel_INT::Accel_INT(uint16_t INT_pin, IMU_typeC *imu) : GPIT(INT_pin) {
  imu_ = imu;
}
...
Gyro_INT::Gyro_INT(uint16_t INT_pin, IMU_typeC *imu) : GPIT(INT_pin) {
  imu_ = imu;
}
...
IMU_typeC::IMU_typeC(IMU_typeC_init_t init, bool useMag)
    : IST8310_(init.IST8310, this), BMI088_(init.BMI088), heater_(init.heater),
  ...
  BMI088_.Read(BMI088_real_data_.gyro, BMI088_real_data_.accel,
               &BMI088_real_data_.temp);
  accel_fliter_1[0] = accel_fliter_2[0] = accel_fliter_3[0] =
      BMI088_real_data_.accel[0];
  accel_fliter_1[1] = accel_fliter_2[1] = accel_fliter_3[1] =
      BMI088_real_data_.accel[1];
  accel_fliter_1[2] = accel_fliter_2[2] = accel_fliter_3[2] =
      BMI088_real_data_.accel[2];
...
void IMU_typeC::Update() {
  ...
  if (accel_temp_update_flag & (1 << IMU_UPDATE_SHFITS)) {
    ...
    BMI088_.temperature_read_over(accel_temp_dma_rx_buf +
  ...
  if (calibrate_ && Temp > heater_param_.temp - 2) {
    ...
    AHRS_update(INS_quat, 0.001f, BMI088_real_data_.gyro,
                BMI088_real_data_.accel, IST8310_real_data_.mag);
...
void IMU_typeC::AHRS_update(float *quat, float time, float *gyro, float *accel,
                            float *mag) {
  UNUSED(time);
  ...
  if (useMag_) {
    ...
    MahonyAHRSupdateIMU(quat, gyro[0], gyro[1], gyro[2], accel[0], accel[1],
                        accel[2]);
  }
}
...
void IMU_typeC::SPI_DMA_enable(uint32_t tx_buf, uint32_t rx_buf,
                               uint16_t ndtr) {
  // disable DMA
  __HAL_DMA_DISABLE(hdma_spi_rx_);
  __HAL_DMA_DISABLE(hdma_spi_tx_);
  ...
  // clear flag
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmarx,
                       __HAL_DMA_GET_TC_FLAG_INDEX(hspi_->hdmarx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmarx,
                       __HAL_DMA_GET_HT_FLAG_INDEX(hspi_->hdmarx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmarx,
                       __HAL_DMA_GET_TE_FLAG_INDEX(hspi_->hdmarx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmarx,
                       __HAL_DMA_GET_DME_FLAG_INDEX(hspi_->hdmarx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmarx,
                       __HAL_DMA_GET_FE_FLAG_INDEX(hspi_->hdmarx));

  __HAL_DMA_CLEAR_FLAG(hspi_->hdmatx,
                       __HAL_DMA_GET_TC_FLAG_INDEX(hspi_->hdmatx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmatx,
                       __HAL_DMA_GET_HT_FLAG_INDEX(hspi_->hdmatx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmatx,
                       __HAL_DMA_GET_TE_FLAG_INDEX(hspi_->hdmatx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmatx,
                       __HAL_DMA_GET_DME_FLAG_INDEX(hspi_->hdmatx));
  __HAL_DMA_CLEAR_FLAG(hspi_->hdmatx,
                       __HAL_DMA_GET_FE_FLAG_INDEX(hspi_->hdmatx));
  // set memory address
...
void DMACallbackWrapper(SPI_HandleTypeDef *hspi) {
  if (__HAL_DMA_GET_FLAG(hspi->hdmarx,
                         __HAL_DMA_GET_TC_FLAG_INDEX(hspi->hdmarx)) != RESET) {
    ...
    if (imu->gyro_update_flag & (1 << IMU_SPI_SHFITS)) {
      ...
      HAL_GPIO_WritePin(imu->BMI088_param_.CS_GYRO_Port,
                        imu->BMI088_param_.CS_GYRO_Pin, GPIO_PIN_SET);
    }
    // accel read over
    if (imu->accel_update_flag & (1 << IMU_SPI_SHFITS)) {
      imu->accel_update_flag &= ~(1 << IMU_SPI_SHFITS);
      imu->accel_update_flag |= (1 << IMU_UPDATE_SHFITS);
      HAL_GPIO_WritePin(imu->BMI088_param_.CS_ACCEL_Port,
                        imu->BMI088_param_.CS_ACCEL_Pin, GPIO_PIN_SET);
    }
