...
ConstrainedPID::ConstrainedPID(float kp, float ki, float kd, float max_iout,
                               float max_out) {
...
float ConstrainedPID::ComputeOutput(float error) {
  ...
  last_err_ = error;
  cumulated_err_ =
      clip<float>(cumulated_err_, -max_iout_ / ki_, max_iout_ / ki_);
...
void ConstrainedPID::Reinit(float kp, float ki, float kd, float max_iout,
                            float max_out) {
  kp_ = kp;
  ki_ = ki;
  kd_ = kd;
