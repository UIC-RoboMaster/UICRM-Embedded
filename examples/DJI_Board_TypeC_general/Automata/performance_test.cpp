...
void RM_RTOS_Default_Task(const void* args){
    ...
    // FSM2
    auto fsm2 = control::AutomataBuilder<S2>()
    ...
    auto fsm3 = control::AutomataBuilder<S3>()
        ...
        .transition<S3::IDLE, S3::RUN>(TRANLOGIC { return ins.template get<0>().get() == S2::Y; })
        .transition<S3::RUN, S3::IDLE>(TRANLOGIC { return ins.template get<0>().get() == S2::X; })
        .build<S3::IDLE>();
    ...
    auto fsm_heavy = control::AutomataBuilder<S_HEAVY>()
        ...
        .transition<S_HEAVY::B, S_HEAVY::A>(TRANLOGIC { return ins.template get<0>().get() < 50; })
        .build<S_HEAVY::A>();
    uint32_t build_end=DWT->CYCCNT;

    const int TEST_COUNT=1000000;
    uint32_t cycle_sum=0, cycle_1[3]={0}, cycle_2[3]={0}, cycle_3[3]={0}, cycle_big[3]={0,0,0}, cycle_heavy[3]={0,0,0}, cycle_switch=0;
    ...
    for(int i=0;i<TEST_COUNT;i++){
        ...
        // FSM1
        switch(sw_fsm1){
            case S1::A: if(input>100) sw_fsm1=S1::B; break;
            case S1::B: if(input>200) sw_fsm1=S1::C; break;
            case S1::C: if(input<50) sw_fsm1=S1::A; break;
        }
        // FSM2
        switch(sw_fsm2){
            case S2::X: if(sw_fsm1==S1::C) sw_fsm2=S2::Y; break;
            case S2::Y: if(sw_fsm1==S1::A) sw_fsm2=S2::X; break;
        }
        // FSM3
        switch(sw_fsm3){
            case S3::IDLE: if(sw_fsm2==S2::Y) sw_fsm3=S3::RUN; break;
            case S3::RUN:  if(sw_fsm2==S2::X) sw_fsm3=S3::IDLE; break;
        }
        // Big FSM
        static int j=0;
        switch(sw_fsm_big){
            case S_BIG::S0: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S1; break;
            case S_BIG::S1: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S2; break;
            case S_BIG::S2: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S3; break;
            case S_BIG::S3: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S4; break;
            case S_BIG::S4: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S5; break;
            case S_BIG::S5: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S6; break;
            case S_BIG::S6: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S7; break;
            case S_BIG::S7: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S8; break;
            case S_BIG::S8: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S9; break;
            case S_BIG::S9: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S10; break;
            case S_BIG::S10: if(big_input>(++j*50)) sw_fsm_big=S_BIG::S11; break;
            case S_BIG::S11: if(big_input<10) sw_fsm_big=S_BIG::S0; break;
        }
        // Heavy FSM
        switch(sw_fsm_heavy){
            case S_HEAVY::A:{
                bool cond=(big_input%3==0)&&(big_input%5!=0)&&(big_input*big_input>1000)
                          &&((big_input^0x55)&0x0F)&&(big_input>123 && big_input<300);
                if(cond) sw_fsm_heavy=S_HEAVY::B;
            } break;
            case S_HEAVY::B: if(big_input<50) sw_fsm_heavy=S_HEAVY::A; break;
        }
        uint32_t te=DWT->CYCCNT;
        cycle_switch+=te-ts;
    }
    uint32_t fsm_cycle_sum =
        cycle_1[2]/TEST_COUNT+cycle_2[2]/TEST_COUNT+cycle_3[2]/TEST_COUNT+cycle_big[2]/TEST_COUNT+cycle_heavy[2]/TEST_COUNT;

    print("CPU frequency: %lu\r\n", cpu_hz);
    print("Build cycles: %lu(%lu ns)\r\n", build_end-build_start, cycleToNanoSec(build_end-build_start));
    print("\r\n");
    print("Common FSM1 avg cycles: %lu(%lu ns)\r\n", cycle_1[2]/TEST_COUNT, cycleToNanoSec(cycle_1[2]/TEST_COUNT));
    print("Common FSM2 avg cycles: %lu(%lu ns)\r\n", cycle_2[2]/TEST_COUNT, cycleToNanoSec(cycle_2[2]/TEST_COUNT));
    print("Common FSM3 avg cycles: %lu(%lu ns)\r\n", cycle_3[2]/TEST_COUNT, cycleToNanoSec(cycle_3[2]/TEST_COUNT));
    print("Big FSM avg cycles: %lu(%lu ns)\r\n", cycle_big[2]/TEST_COUNT, cycleToNanoSec(cycle_big[2]/TEST_COUNT));
    print("Heavy FSM avg cycles: %lu(%lu ns)\r\n", cycle_heavy[2]/TEST_COUNT, cycleToNanoSec(cycle_heavy[2]/TEST_COUNT));
    print("\r\n");
    print("5 FSMs avg cycles: %lu(%lu ns)\r\n", fsm_cycle_sum, cycleToNanoSec(fsm_cycle_sum));
    print("5 Switch-cases avg cycles: %lu(%lu ns)\r\n", cycle_switch/TEST_COUNT, cycleToNanoSec(cycle_switch/TEST_COUNT));
    print("\r\n");
    print("1 FSM avg cycles: %lu(%lu ns)\r\n", fsm_cycle_sum/5, cycleToNanoSec(fsm_cycle_sum/5));
    print("1 Switch-case avg cycles: %lu(%lu ns)\r\n", cycle_switch/5/TEST_COUNT, cycleToNanoSec(cycle_switch/5/TEST_COUNT));
}