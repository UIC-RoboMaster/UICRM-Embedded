...
void RM_RTOS_Default_Task(const void* args) {
    ...
    auto fsm_big = control::AutomataBuilder<S_BIG>()
                       ...
                       .transition<S_BIG::S0, S_BIG::S1>(TRANLOGIC { return ins.template get<0>().get() > 50; })

                       .transition<S_BIG::S1, S_BIG::S2>(TRANLOGIC { return ins.template get<0>().get() > 100; })

                       .transition<S_BIG::S2, S_BIG::S3>(TRANLOGIC { return ins.template get<0>().get() > 150; })

                       .transition<S_BIG::S3, S_BIG::S4>(TRANLOGIC { return ins.template get<0>().get() > 200; })

                       .transition<S_BIG::S4, S_BIG::S5>(TRANLOGIC { return ins.template get<0>().get() > 250; })

                       .transition<S_BIG::S5, S_BIG::S6>(TRANLOGIC { return ins.template get<0>().get() > 300; })

                       .transition<S_BIG::S6, S_BIG::S7>(TRANLOGIC { return ins.template get<0>().get() > 350; })

                       .transition<S_BIG::S7, S_BIG::S8>(TRANLOGIC { return ins.template get<0>().get() > 400; })

                       .transition<S_BIG::S8, S_BIG::S9>(TRANLOGIC { return ins.template get<0>().get() > 450; })

                       .transition<S_BIG::S9, S_BIG::S10>(TRANLOGIC { return ins.template get<0>().get() > 500; })

                       .transition<S_BIG::S10, S_BIG::S11>(TRANLOGIC { return ins.template get<0>().get() > 550; })

                       .transition<S_BIG::S11, S_BIG::S0>(TRANLOGIC { return ins.template get<0>().get() < 10; })

                       .build<S_BIG::S0>();

    // Heavy FSM
    auto fsm_heavy =
        control::AutomataBuilder<S_HEAVY>()
    ...
    uint32_t cycle_sum = 0, cycle_1[3] = {0}, cycle_2[3] = {0}, cycle_3[3] = {0}, cycle_big[3] = {0, 0, 0},
             cycle_heavy[3] = {0, 0, 0}, cycle_switch = 0;
    ...
    uint32_t fsm_cycle_sum = cycle_1[2] / TEST_COUNT + cycle_2[2] / TEST_COUNT + cycle_3[2] / TEST_COUNT +
                             cycle_big[2] / TEST_COUNT + cycle_heavy[2] / TEST_COUNT;
    ...
    print(
        "5 Switch-cases avg cycles: %lu(%lu ns)\r\n",
    ...
    print(
        "1 Switch-case avg cycles: %lu(%lu ns)\r\n",
