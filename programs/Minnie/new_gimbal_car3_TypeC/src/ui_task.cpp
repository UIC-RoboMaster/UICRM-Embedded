...
void uiTask(void* arg) {
    ...
    // Initialize chassis power GUI
    char batteryStr[15] = "CHASSIS_POWER";
    batteryGUI = new communication::CapGUI(UI, batteryStr);
    osDelay(110);
    batteryGUI->InitName();
    osDelay(110);
    ...
    char initModeStr[15] = "Init False!";
    initGUI = new communication::StringGUI(UI, initModeStr, 1500, 520, UI_Color_Pink);
    ...
    // BoolEdgeDetector* keyboard_C_edge = new BoolEdgeDetector(false);
    ...
    while (true) {
        ...
        // 更新电源百分比（后期是超级电容剩余点亮
        batteryGUI->Update(referee->game_robot_status.chassis_power_limit);
        osDelay(UI_OS_DELAY);
        ...
        BombchamberGUI->Update(BombchamberStr, modeColor);
        osDelay(UI_OS_DELAY);
