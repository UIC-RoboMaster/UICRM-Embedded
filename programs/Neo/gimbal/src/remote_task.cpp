...
void remoteTask(void* arg) {
    ...
    auto activate_aut =
        control::AutomataBuilder<ActivateStates>()
    ...
    auto remote_mode_aut =
        control::AutomataBuilder<RemoteMode>()
            .item<control::AutomataInputEdge>(dbus->swr)
            .item<control::AutomataInputEdge>(bool{})  // mode_change_key
            .transition<REMOTE_MODE_AUTOPILOT, REMOTE_MODE_FOLLOW>(
                tranlogic_next_remote_mode_trigger)
    ...
    */
    ...
    auto fric_wheel_aut =
        control::AutomataBuilder<ShootFricMode>()
            .item<control::AutomataInputEdge>(dbus->swl)
            .item<control::AutomataInputEdge>(bool{})  // friction_key
            .transition<SHOOT_FRIC_MODE_PREPARING, SHOOT_FRIC_MODE_STOP>(
                tranlogic_fric_wheel_trigger)
            .transition<SHOOT_FRIC_MODE_PREPARED, SHOOT_FRIC_MODE_STOP>(
                tranlogic_fric_wheel_trigger)
            .transition<SHOOT_FRIC_MODE_STOP, SHOOT_FRIC_MODE_PREPARING>(
                tranlogic_fric_wheel_trigger)
    ...
    auto tranlogic_shooting_condition = TRANLOGIC {
        ...
        return (swl.get() == remote::DOWN || shoot_key.get()) &&
               fric_state.get() == SHOOT_FRIC_MODE_PREPARED && referee_permit.get();
    };
    auto shoot_aut =
        control::AutomataBuilder<ShootMode>()
            .item<control::AutomataInputEdge>(dbus->swl)
            ...
            .transition<SHOOT_MODE_SINGLE, SHOOT_MODE_STOP>(tranlogic_shooting_condition,
                                                            control::ReverseTag{})
            .transition<SHOOT_MODE_BURST, SHOOT_MODE_STOP>(tranlogic_shooting_condition,
                                                           control::ReverseTag{})
    ...
    auto tranlogic_bullet_cap_trigger = TRANLOGIC {
        const auto& swl = COMPONENT(0);
        const auto& fric_state = COMPONENT(1);
        return swl.downEdge() && swl.get() == remote::DOWN &&
               fric_state.get() == SHOOT_FRIC_MODE_STOP;
    };
    auto bullet_cap_aut =
        control::AutomataBuilder<BulletCapMode>()
            .item<control::AutomataInputEdge>(dbus->swl)
            .item<control::AutomataInputRaw>(SHOOT_FRIC_MODE_STOP)
    ...
    while (true) {
        ...
        remote_mode_aut.input(
            std::make_tuple(dbus->swr, static_cast<bool>(dbus->keyboard.bit.SHIFT)));
        ...
        shoot_aut.input(std::make_tuple(dbus->swl, shoot_fric_wheel_mode,
                                        is_referee_shoot_available, dbus->mouse.l));
