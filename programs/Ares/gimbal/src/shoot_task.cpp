...
void jam_callback(void* args) {  // 取消卡弹检测——
    UNUSED(args);
            ...
            switch (shoot_load_mode) {
                ...
                case SHOOT_MODE_SINGLE:
                    ...
                    if (last_shoot_mode != SHOOT_MODE_SINGLE) {
                        if (steering_motor->IsHolding()) {
                            steering_motor->SetTarget(steering_motor->GetTarget() + 2 * PI / 6, false);
                            if (steering_up->IsHolding()) {
                                steering_up->SetTarget(steering_up->GetTarget() + 2 * PI / 4, true);
                            }
                        }
                        shoot_load_mode = SHOOT_MODE_PREPARED;
                    }
...
void init_shoot() {
    ...
    flywheel_left->SetMode(driver::DjiMotorBase::OMEGA | driver::DjiMotorBase::INVERTED);
    ...
    steering_up->ReInitPID(enhance_theta_pid_init, driver::DjiMotorBase::THETA);
    steering_up->ReInitPID(enhance_omega_pid_init, driver::DjiMotorBase::OMEGA);
    steering_up->SetMode(driver::DjiMotorBase::THETA | driver::DjiMotorBase::OMEGA);
