...
void uiTask(void* arg) {
    ...
    // Initialize supercapacitor / battery GUI
    ...
    char turboOffStr[15] = "        ";
    turboGUI = new communication::StringGUI(UI, turboOffStr, 840, 730, UI_Color_Orange, 30);
    ...
    BoolEdgeDetector* steer_up_motor_check_edge = new BoolEdgeDetector(false);
    ...
    while (true) {
        ...
        // Update turbo shoot status GUI
        turboEdgeDetector->input(turbo_shoot);
        if (turboEdgeDetector->edge()) {
            char* turboStr = turbo_shoot ? turboOnStr : turboOffStr;
            turboGUI->Update(turboStr, UI_Color_Orange);
            osDelay(UI_OS_DELAY);
        }
        ...
        // Offline diagnostics
        {
            ...
            steer_up_motor_check_edge->input(steering_up->IsOnline());
            fl_motor_check_edge->input(flywheel_left->IsOnline());
            fr_motor_check_edge->input(flywheel_right->IsOnline());
            ...
            if (steer_up_motor_check_edge->negEdge()) {
                strcpy(diagStr, "STEER UP OFFLINE     ");
                diagGUI->Update(diagStr, UI_Delay, UI_Color_Pink);
            }
            if (fl_motor_check_edge->negEdge()) {
                strcpy(diagStr, "FL FLYWHEEL OFFLINE  ");
                diagGUI->Update(diagStr, UI_Delay, UI_Color_Pink);
            }
            if (fr_motor_check_edge->negEdge()) {
                strcpy(diagStr, "FR FLYWHEEL OFFLINE  ");
                diagGUI->Update(diagStr, UI_Delay, UI_Color_Pink);
            }
        ...
        // V key to refresh UI (via refereerc VT13 link)
        ...
        // C key to clear diagnostics messages
        if (dbus->IsOnline()) {
