...
/**
 * @brief 核心安全检查：处理 KILL 模式并确保电机使能
 * @return true 如果处于 KILL 模式，false 正常运行
 */
static bool check_safety_and_enable() {
...
void gimbalTask(void* arg) {
    ...
    // IMU init
    int i = 0;
    bool cali_started = false;

    while (i < 5000 || !imu->DataReady() || !imu->CaliDone()) {
        if (check_safety_and_enable()) {
            osDelay(GIMBAL_OS_DELAY);
            continue;
        }

        // 触发校准
        if (imu->DataReady() && !cali_started) {
    ...
    while (true) { // TODO dup?
        ...
        } else if (refereerc->IsOnline()) { // referee control
