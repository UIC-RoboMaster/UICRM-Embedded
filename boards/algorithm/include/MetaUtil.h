...
namespace TemplateMetaUtil {
    ...
    struct AppendUnique<ValueList<Vs...>, V> {
        using type = std::conditional_t<Contains<V, ValueList<Vs...>>::value, ValueList<Vs...>, ValueList<Vs..., V>>;
    };
    ...
    struct Filter<State, T, Rest...> {
      ...
      public:
        using type =
            std::conditional_t<T::from == State, decltype(std::tuple_cat(std::tuple<T>{}, rest_type{})), rest_type>;
    };
    /*Filter*/
}  // namespace TemplateMetaUtil
