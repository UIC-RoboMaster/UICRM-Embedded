...
#define __ADERNAL_CANID_SAFE  0x001 // 安全提示帧
	...
	Usr_CAN_TxSuccess_Typedef;// CAN是否发送成功
	...
	typedef struct
	{
		float Voltage_NoESR; // 未ESR修正后的电容组电压								/*!< ESR-corrected voltage value */

		float Power_Battery; // 电源输入功率						/*!< Bus power */

		uint8_t Work_Sentry1; // 工作强度1							/*!< Work sentry 1,point to curent limit ability */

		uint8_t Work_Sentry2; // 工作强度2							/*!< Work sentry 2,point to power limit ability */
	}
	...
	public:
		...
		const Adernal_Fb_Typedef& getFeedback() const { return feedback_; }
		const Adernal_SafetyLevel_Typedef* getSafetyLevels() const { return safety_levels_; }

		// 状态更新标志
		bool hasNewFeedback() const { return new_feedback_; }
		bool hasNewSafetyInfo() const { return new_safety_; }
		bool hasNewReadyStatus() const { return new_ready_; }

		// 清除状态更新标志
		void clearFeedbackFlag() { new_feedback_ = false; }
		void clearSafetyFlag() { new_safety_ = false; }
		void clearReadyFlag() { new_ready_ = false; }
