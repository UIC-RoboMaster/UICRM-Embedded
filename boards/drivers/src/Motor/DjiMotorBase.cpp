...
namespace driver {
    ...
    DjiMotorBase::DjiMotorBase(bsp::CAN* can, uint16_t rx_id, uint16_t tx_id) : CanMotorBase(30) {
        can_ = can;
        rx_id_ = rx_id;
        tx_id_ = tx_id;
    ...
    void DjiMotorBase::AngleTracking() {
        ...
        state_.encoder_cumulated_angle = state_.encoder_cumulated_turns * 2 * PI + state_.encoder_relative_angle;
        ...
        state_.output_cumulated_angle = state_.output_cumulated_turns * 2 * PI + state_.output_relative_angle;

        // absolute 模式：对外输出限制在 [0, 2π]；相对模式：输出多圈累计角
        state_.output_shaft_theta = state_.absolute_mode ? state_.output_relative_angle : state_.output_cumulated_angle;
    ...
    void DjiMotorBase::SetTorqueFeedforward(float torque_nm) {
        if (torque_constant_ > 0 && max_current_amp_ > 0) {
            state_.torque_feedforward = linear_remap(
                torque_nm / torque_constant_,
                -max_current_amp_,
                max_current_amp_,
    ...
    void DjiMotorBase::SetTorque(float torque_nm, bool override) {
        ...
        RM_ASSERT_TRUE(
            state_.mode & CURRENT,
            "SetTorque requires CURRENT mode; use SetTorqueFeedforward in cascade control"
        );
        SetTarget(
            linear_remap(
                torque_nm / torque_constant_,
    ...
    void DjiMotorBase::ErrorCallbackWrapper(void* instance, control::ConstrainedPID::PID_ErrorHandler_t type) {
        UNUSED(type);
    ...
    void DjiMotorBase::RegisterPreOutputCallback(DjiMotorBase::callback_t callback, void* instance) {
        pre_output_callback_ = callback;
        pre_output_callback_instance_ = instance;
    }

    void DjiMotorBase::RegisterPostOutputCallback(DjiMotorBase::callback_t callback, void* instance) {
    ...
    void Motor3508::UpdateData(const uint8_t data[]) {
        ...
        state_.current = linear_remap<int16_t, float>(
            state_.raw_current,
            -Motor3508Config::MAX_RAW_CURRENT,
            Motor3508Config::MAX_RAW_CURRENT,
    ...
    void Motor6020::UpdateData(const uint8_t data[]) {
        ...
        // 映射 omega 角速度为 rad/s
        state_.omega =
            (state_.raw_omega * 2 * PI / 60) * input_speed_filter_ + state_.omega * (1 - input_speed_filter_);
        // GM6020 转矩电流反馈 raw_current ∈ [-16384, 16384] 对应 [-3A, 3A]
        state_.current = linear_remap<int16_t, float>(
            state_.raw_current,
            -Motor6020Config::MAX_RAW_CURRENT,
            Motor6020Config::MAX_RAW_CURRENT,
            -Motor6020Config::MAX_CURRENT,
            Motor6020Config::MAX_CURRENT
        );
        state_.torque = state_.current * torque_constant_;
    ...
    void Motor2006::UpdateData(const uint8_t data[]) {
        ...
        state_.current = linear_remap<int16_t, float>(
            state_.raw_current,
            -Motor2006Config::MAX_RAW_CURRENT,
            Motor2006Config::MAX_RAW_CURRENT,
    ...
    void Dm1to4::UpdateData(const uint8_t data[]) {
        ...
        // C620 转矩电流反馈 raw_current ∈ [-16384, 16384] 对应 [-10.5A, 10.5A]
        state_.current = linear_remap<int16_t, float>(
            state_.raw_current,
            -Dm1to4Config::MAX_RAW_CURRENT,
            Dm1to4Config::MAX_RAW_CURRENT,
    ...
    void Dm1to4::PrintData() const {
        ...
        print("output shaft omega: % .4f ", GetOutputShaftOmega());
        print("raw temperature: %3d ", state_.raw_temperature);
        print("raw current get: % d ", state_.raw_current);
        print("raw error: 0x%02X\r\n", error_);
    }
