...
namespace driver {
    ...
    DmMotorBase::DmMotorBase(
        bsp::CAN* can, uint16_t master_id, uint16_t motor_can_id, DmControlMode mode, const DmMotorConfig& config
    )
        : CanMotorBase(30) {
        can_ = can;
        rx_id_ = master_id;
    ...
    void DmMotorBase::TransmitOutput() {
        uint8_t data[8] = {0};
        const uint8_t dlc = state_.tx.Pack(
            data,
            state_.mode,
            config_.angle_max,
            config_.omega_max,
    ...
    void DMMotor4310::PrintData() const {
        ...
        print(
            "mode:%u status:%s(0x%X) rx_id:0x%03X tx_id:0x%03X last_us:%u\r\n",
            static_cast<uint16_t>(state_.mode),
            name_of(GetControlStatus()),
            static_cast<uint8_t>(GetControlStatus()),
            rx_id_,
            tx_id_,
        ...
        print("--- RX raw ---\r\n");
        print(
            "motor_id:%u raw_theta:%u raw_omega:%u raw_torque:%u\r\n",
            state_.rx.motor_id,
            state_.rx.raw_theta,
            state_.rx.raw_omega,
            state_.rx.raw_torque
        );
        ...
        print("--- TX target ---\r\n");
        print(
            "p_des:% .4f v_des:% .4f kp:% .2f kd:% .2f t_ff:% .4f\r\n",
            state_.tx.p_des,
            state_.tx.v_des,
            state_.tx.kp,
            state_.tx.kd,
