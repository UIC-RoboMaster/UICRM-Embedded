...
#define __ADERNAL_CANID_CTRL 0x004  // 控制帧ID
    ...
    Usr_CAN_TxSuccess_Typedef Adernal_SuperCap::Usr_CAN_TxMsg(
        Usr_CAN_TxHeader_Typedef* Usr_CAN_TxHeader, uint8_t* TxData) {
        ...
        if (Usr_CAN_TxHeader->IDtype == USR_CAN_IDTYPE_STANDARD) {
            // 标准帧发送
            ret =
                can_->Transmit(Usr_CAN_TxHeader->ID, TxData, Usr_CAN_TxHeader->DataLength);
        } else if (Usr_CAN_TxHeader->IDtype == USR_CAN_IDTYPE_EXTENDED) {
            // 扩展帧发送
            ret = can_->TransmitExtend(Usr_CAN_TxHeader->ID, TxData,
    ...
    Usr_CAN_TxSuccess_Typedef Adernal_SuperCap::Adernal_Init(Adernal_Init_Typedef Adernal_Init) {
     ...
     * @retval Usr_CAN_TxSuccess_Typedef.
     */
    Usr_CAN_TxSuccess_Typedef Adernal_SuperCap::Adernal_Ctrl(Adernal_Ctrl_Typedef* Adernal_Ctrl) {
