...
#define __ADERNAL_CANID_CTRL  0x004 // 控制帧ID
    ...
    Usr_CAN_TxSuccess_Typedef Usr_CAN_TxMsg(Usr_CAN_TxHeader_Typedef *Usr_CAN_TxHeader, uint8_t *TxData)
    {
        ...
        if(Usr_CAN_TxHeader->IDtype == USR_CAN_IDTYPE_STANDARD) {
            ...
            // 扩展帧发送
            ret = can_instance->TransmitExtend(
                Usr_CAN_TxHeader->ID,
                TxData,
                Usr_CAN_TxHeader->DataLength
            );
    ...
    /**
      ...
      * @retval Adernal_StdbyState_Typedef.Adernal_StdbyState_OK point to standingby is ok,Adernal_StdbyState_Fail point to not standing by,Adernal_StdbyState_Error point to cannor parse frame.
      */
    uint8_t Adernal_StdbyFrame_Unpack(Usr_CAN_RxHeader_Typedef* Usr_CAN_RxHeader, uint8_t* RxData)
    {
        /*Adernal_StdbyFrame_Unpack 是用于解析待机状态反馈帧的专用函数，主要用于处理 CAN 接收中断中获取的待机状态数据。
         * 返回超电控制器是否就绪*/
        // 判断帧头是否正确
    ...
    /**
      ...
      * @retval Adernal_FbState_Typedef.Adernal_FbState_OK point to parse is ok,Adernal_FbState_Error point to cannor parse frame.
      */
    Adernal_FbState_Typedef Adernal_FbFrame_Unpack(Usr_CAN_RxHeader_Typedef *Usr_CAN_RxHeader, uint8_t *RxData,Adernal_Fb_Typedef *Adernal_Fb)
    {
        /*Adernal_FbFrame_Unpack 是用于解析超级电容反馈数据帧的核心函数，主要处理电压和功率等关键参数的实时反馈*/
        // 判断帧头是否正确
    ...
    {
        /*Adernal_Ctrl 函数是用于发送超级电容控制指令帧的核心接口，通过 CAN 总线向超级电容模块发送功率设定、保护阈值和工作模式等控制参数*/
        // 创建 CAN 发送帧结构体
        Usr_CAN_TxHeader_Typedef Usr_CAN_TxHeader;
    ...
    /**
      ...
      * @param  Usr_CAN_RxHeader_Typedef *Usr_CAN_RxHeader,The frame header of the CAN packet.
      * @param  uint8_t *RxData,The content of the CAN packet.
      * @param  Adernal_SafetyLevel_Typedef *Adernal_SafetyLevel,Point to an 8-byte array to store parse results.
      * @retval Adernal_SafetyState_Typedef.Adernal_SafetyState_OK point to parse is ok,Adernal_SafetyState_Error point to cannor parse frame.
      */
    Adernal_SafetyState_Typedef Adernal_Safety_Unpack(Usr_CAN_RxHeader_Typedef *Usr_CAN_RxHeader, uint8_t *RxData, Adernal_SafetyLevel_Typedef *Adernal_SafetyLevel)
    {
        /*Adernal_Safety_Unpack 是用于解析超级电容安全状态帧的关键函数，主要用于实时监测电容模块的故障和安全等级信息。*/
        // 判断帧头是否正确
    ...
    bool Adernal_SuperCap::setControl(uint8_t expect_power,
                                       Adernal_CtrlMode_Typedef mode,
    ...
    void Adernal_SuperCap::handleSafetyFrame(const uint8_t data[]) {
        ...
        if (Adernal_Safety_Unpack(&rx_header, (uint8_t*)data, safety_levels_) == Adernal_SafetyState_OK) {
            new_safety_ = true;
        }
    }
}