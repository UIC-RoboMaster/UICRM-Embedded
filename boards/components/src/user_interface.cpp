...
namespace communication {
     ...
     */
    void UserInterface::LineDraw(graphic_data_t* image,
                                 const char name[3],
                                 uint32_t graphOperate,
     ...
     * @param end_y ending y coordinate
     */
    void UserInterface::RectangleDraw(graphic_data_t* image,
                                      const char name[3],
     ...
     */
    void UserInterface::CircleDraw(graphic_data_t* image,
                                   const char name[3],
                                   uint32_t graph_operate,
                                   uint32_t graph_layer,
                                   uint32_t graph_color,
     ...
     */
    void UserInterface::EllipseDraw(graphic_data_t* image,
                                    const char name[3],
                                    uint32_t graph_operate,
                                    uint32_t graph_layer,
                                    uint32_t graph_color,
     ...
     * @param y_length vertical semi-axis length
     */
    void UserInterface::ArcDraw(graphic_data_t* image,
                                const char name[3],
                                uint32_t graph_operate,
                                uint32_t graph_layer,
                                uint32_t graph_color,
     ...
     */
    void UserInterface::FloatDraw(graphic_data_t* image,
                                  const char name[3],
                                  uint32_t graph_operate,
                                  uint32_t graph_layer,
                                  uint32_t graph_color,
    ...
    void UserInterface::IntDraw(graphic_data_t* image,
                                const char name[3],
                                uint32_t graph_operate,
                                uint32_t graph_layer,
                                uint32_t graph_color,
     ...
     * @param start_y starting y coordinate
     */
    void UserInterface::CharDraw(graphic_data_t* image,
                                 const char name[3],
                                 uint32_t graph_operate,
                                 uint32_t graph_layer,
                                 uint32_t graph_color,
    ...
    ChassisGUI::ChassisGUI(
        UserInterface* UI, int16_t chassis_X, int16_t chassis_Y, int16_t speed_center_X, int16_t speed_center_Y)
        : UI_(UI) {
    ...
    void ChassisGUI::Init() {
        ...
        UI_->LineDraw(&speed_x_axis_,
                      "sx",
                      UI_Graph_Add,
                      3,
                      UI_Color_Pink,
                      2,
                      speed_center_X_ - speed_circle_R_,
                      speed_center_Y_,
                      speed_center_X_ + speed_circle_R_,
                      speed_center_Y_);
        UI_->LineDraw(&speed_y_axis_,
                      "sy",
                      UI_Graph_Add,
                      3,
                      UI_Color_Pink,
    ...
    void ChassisGUI::Init2() {
        UI_->IntDraw(&speed_x_val_,
                     ...
                     10,
                     2,
                     speed_center_X_ + speed_circle_R_ + 8,
                     speed_center_Y_ - 2,
                     0);
        UI_->IntDraw(&speed_y_val_,
                     "vy",
                     UI_Graph_Add,
                     2,
                     UI_Color_Green,
                     ...
                     2,
                     speed_center_X_ - 5,
                     speed_center_Y_ - speed_circle_R_ - 10,
                     0);
    ...
    void ChassisGUI::Update(float speed_x, float speed_y, float relative) {
        ...
        UI_->CircleDraw(&speed_center_,
                        ...
                        speed_center_Y_ + (int16_t)(speed_y * speed_circle_R_),
                        5);
        UI_->IntDraw(&speed_x_val_,
                     "vx",
                     UI_Graph_Change,
    ...
    CrossairGUI::CrossairGUI(UserInterface* UI,
                             int16_t centerX,
                             int16_t centerY,
                             int16_t x1,
                             int16_t x2,
    ...
    graphic_data_t Bar::Init() {
        ...
        communication::UserInterface::LineDraw(&bar_,
                                               name_,
                                               UI_Graph_Add,
                                               0,
                                               color_,
                                               barHeight_ - 10,
                                               barStartX_ + barWidth_ / 2,
                                               barStartY_ + 5,
                                               barStartX_ + barWidth_ / 2,
                                               barStartY_ + barHeight_ - 5);
        return bar_;
    }
    ...
    graphic_data_t Bar::InitFrame() {
        ...
        communication::UserInterface::RectangleDraw(&barFrame_,
                                                    name_frame_,
                                                    UI_Graph_Add,
                                                    0,
                                                    frame_color_,
                                                    ...
                                                    barStartX_,
                                                    barStartY_,
                                                    barStartX_ + barWidth_,
                                                    barStartY_ + barHeight_);
        return barFrame_;
    }
    ...
    graphic_data_t Bar::Update(float percent, int8_t color) {
        ...
        if (isVertical_) {
            ...
            communication::UserInterface::LineDraw(&bar_,
                                                   ...
                                                   barStartX_ + 5 + (barWidth_ - 10) * percent_,
                                                   barStartY_ + barHeight_ / 2);
        }
        return bar_;
    }
    ...
    void GimbalGUI::Init() {
        UI_->CircleDraw(&speed_circle_,
                        ...
                        UI_Color_Yellow,
                        2,
                        gimbal_speed_center_X_,
                        gimbal_speed_center_Y_,
                        gimbal_speed_circle_R_);
        UI_->CircleDraw(&speed_center_,
                        "gd",
                        UI_Graph_Add,
                        2,
        ...
        UI_->LineDraw(&speed_x_axis_,
                      ...
                      UI_Color_Pink,
                      2,
                      gimbal_speed_center_X_ - gimbal_speed_circle_R_,
                      gimbal_speed_center_Y_,
                      gimbal_speed_center_X_ + gimbal_speed_circle_R_,
                      gimbal_speed_center_Y_);
        UI_->LineDraw(&speed_y_axis_,
                      ...
                      2,
                      gimbal_speed_center_X_,
                      gimbal_speed_center_Y_ - gimbal_speed_circle_R_,
                      gimbal_speed_center_X_,
                      gimbal_speed_center_Y_ + gimbal_speed_circle_R_);
        ...
        UI_->IntDraw(&speed_y_val_,
                     ...
                     gimbal_speed_center_Y_ - gimbal_speed_circle_R_ - 10,
                     0);
        UI_->GraphRefresh(7,
                          speed_circle_,
                          speed_center_,
    ...
    void GimbalGUI::Update(float vpitch, float vyaw, float pitch, float yaw, bool flags) {
        ...
        UI_->CircleDraw(&speed_center_,
                        ...
                        gimbal_speed_center_Y_ + (int16_t)(vpitch * gimbal_speed_circle_R_),
                        5);
        UI_->IntDraw(&speed_x_val_,
                     "gvx",
                     UI_Graph_Change,
    ...
    StringGUI::StringGUI(communication::UserInterface* UI,
                         char* string_content,
                         int16_t string_X,
                         int16_t string_Y,
                         int8_t color,
    ...
    void DiagGUI::Update(char* String, delay_t delay_function, int8_t color) {
        ...
        if (diag_string_[count_] == nullptr) {
            diag_string_[count_] = new StringGUI(UI_, String, diag_X_, diag_Y_ - count_ * 20, color, 10, name);
        }
